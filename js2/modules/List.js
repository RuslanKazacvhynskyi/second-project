...
localStorage.setItem('lastOpenedList', event.target.getAttribute('href'))
    }
});
...
if(btn) {
btn.innerText = this.form.getAttribute('data-edit') ? btn.innerText.replace('Edit', 'Add') : btn.innerText.replace('Add', 'Edit')
};
}
...
let target = window.location.hash || localStorage.getItem('lastOpenedList');
